	...
	var Menu = function () {
	  ...
	  _createClass(Menu, [{
	    ...
	    value: function playSong() {
	      ...
	      if (this.selected === 0) {
	        var song = new _song4.default('assets/easy.mp3', 95, 120, 3185, 100, text, 144, 20.345);
	        var game = new _game4.default(song);
	        game.start();
	      }
	      if (this.selected === 1) {
	        var _song = new _song4.default('assets/medium.mp3', 88, 200, 2725, 100, testText, 258, 10.85);
	        var _game = new _game4.default(_song);
	        _game.start();
	      }
	      if (this.selected === 2) {
	        var _song2 = new _song4.default('assets/hard.mp3', 125, 300, 5750, 100, testText, 480, 13.23);
	        var _game2 = new _game4.default(_song2);
	        _game2.start();
	      }
	    }
	  }]);
...
/***/ function(module, exports) {
	...
	var Song = function () {
	  function Song(path, time, bpm, offset, hSpace, text, beats, timeOffset) {
	    ...
	    // intervals
	    // 100, 120, 125, 150
	    // TODO: Find out non interval method for drawing letters to use more BPMs
	    this.path = path;
	    this.time = time;
	    this.bpm = bpm;
	    this.offset = offset;
