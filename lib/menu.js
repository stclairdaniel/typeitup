...
class Menu {
  ...
  playSong (selected) {
    ...
    if (selected === 0) {
      const song = new Song('http://res.cloudinary.com/loudsounds/video/upload/v1473962530/easy_c6pnms.mp3', 95, 120, 3185, 100, text, 144, 20.345);
      const game = new Game(song, 'assets/easy.jpg');
      game.start();
    }
    if (selected === 1) {
      const song = new Song('http://res.cloudinary.com/loudsounds/video/upload/v1473962527/medium_fppvup.mp3', 88, 200, 2725, 100, text, 255, 10.85);
      const game = new Game(song, 'assets/medium.jpg');
      game.start();
    }
    if (selected === 2) {
      const song = new Song('http://res.cloudinary.com/loudsounds/video/upload/v1473962535/hard_xkhdjp.mp3', 100, 300, 2365, 100, text, 464, 6.55);
